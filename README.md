...
В истории git сохранены **атомарные** коммиты по этапам: инфраструктура и зависимости → реализация API → тесты → CI/CD и документация. После значимых этапов выполнялся **`git push`** в `origin` (см. `git log`).

Отдельно зафиксирован **мини-цикл TDD** для валидации названий досок: коммит `test(red): expect board titles...` (намеренно падающий тест) и следующий `feat: trim board titles...` (**green**). На промежуточной ревизии CI мог быть красным — это ожидаемо для демонстрации красной фазы.
